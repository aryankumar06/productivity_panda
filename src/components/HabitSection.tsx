                  ...
                  >
                    <div className="flex items-center bg-[#232323] rounded-md border border-[#2a2a2a] px-2">
                        <input
                            ...
                            autoFocus
                        />
                        <button onClick={() => { setShowSearch(false); setSearchQuery(''); }} className="text-gray-400 hover:text-white" aria-label="Clear search">
                            <X className="w-3 h-3" />
                        </button>
                    </div>
                  </motion.div>
                    ...
                    <th key={habit.id} className="text-left px-6 py-3 text-sm font-medium text-gray-400 border-r border-[#2a2a2a] min-w-[140px] group relative">
                        <div className="flex items-center justify-between">
                            ...
                            <div className="opacity-0 group-hover:opacity-100 flex gap-1 absolute right-2 bg-[#1a1a1a] p-1 rounded-md shadow-sm border border-[#333] z-20">
                                <button onClick={() => { setEditingHabit(habit); setFormData({...habit}); setShowForm(true); }} className="hover:text-blue-400 p-1" aria-label={`Edit ${habit.name}`} title={`Edit ${habit.name}`}><Edit2 className="w-3 h-3"/></button>
                                <button onClick={() => deleteHabit(habit.id)} className="hover:text-red-400 p-1" aria-label={`Delete ${habit.name}`} title={`Delete ${habit.name}`}><Trash2 className="w-3 h-3"/></button>
                            </div>
                        </div>
                    </th>
                ))}
                        ...
                        <td key={`${habit.id}-${date}`} className="px-6 py-4 border-r border-[#2a2a2a]">
                            <div className="flex justify-start">
                             <Checkbox 
                                 ...
                                 aria-label={`Toggle ${habit.name} on ${format(date, 'EEEE, MMMM do')}`}
                             />
                            </div>
                        </td>
                    ))}
