...
export function PipelineJourney({
  ...
  return (
    ...
    >
      ...
      {COLUMNS.map((col, i) => {
        const flash =
          col.id === "in_progress" ? progFlash : col.id === "done" ? doneFlash : 0;
        return (
          <div
            key={col.id}
            style={{
      ...
      >
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ fontFamily: MONO_FAMILY, fontSize: 11, opacity: 0.5 }}>REMO-128</div>
          <div style={{ width: 8, height: 8, borderRadius: 999, background: accentColor, boxShadow: `0 0 12px ${accentColor}` }} />
        </div>
        <div style={{ fontSize: 16, fontWeight: 600, letterSpacing: "-0.01em" }}>{cardLabel}</div>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: "auto" }}>
          <div style={{ fontFamily: MONO_FAMILY, fontSize: 12, opacity: timerOpacity, color: accentColor }}>
            00:{String(timerSecs).padStart(2, "0")}
          </div>
          <div style={{ fontSize: 11, opacity: 0.4 }}>assignee</div>
        </div>
      </div>
      ...
      {confettiFrame > 0 &&
        Array.from({ length: PARTICLE_COUNT }).map((_, i) => {
          ...
          const color = colorPick < 0.33 ? accentColor : colorPick < 0.66 ? "#fbbf24" : "#60a5fa";
          const lifespan = 50;
          const t = Math.min(confettiFrame / lifespan, 1);
          const eased = 1 - (1 - t) * (1 - t);
          const px = doneCenter.x + Math.cos(angle) * dist * eased + drift * t;
          const py = doneCenter.y + Math.sin(angle) * dist * eased + t * t * 220;
